...
///              | change the Operator.
/// 05/18 | Done | Navier-Stokes Equation.  
/// 05/20 | Done | Adding mkdir.
///
///================================================================
/// Reference : 
///     
...
const string  Dir="Data" , prob = "NSE_test1";
...
void Initial_Value( vector<ConsState>& u0 ,vector<double>& x , vector<double>& y){
    ...
    for (int i=0 ; i<Nx ; i++){
        for (int j=0 ; j<Ny ; j++){
            ...
            u0[idx(i, j)].rho = exp(-r/4.0);
            u0[idx(i, j)].mu  = -1e0*y[j]/sqrt(r)*sin(sqrt(r) * M_PI) * u0[idx(i, j)].rho;
            u0[idx(i, j)].mv  =  1e0*x[i]/sqrt(r)*sin(sqrt(r) * M_PI) * u0[idx(i, j)].rho;
            u0[idx(i, j)].mw  = 0.0 * u0[idx(i, j)].rho;
        }    
    }
