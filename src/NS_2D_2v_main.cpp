...
void Initial_Value( vector<vector<ConsState>>& u0 ,vector<double>& x , vector<double>& y){
    ...
    for (int i=0 ; i<Nx ; i++){
        for (int j=0 ; j<Ny ; j++){
            ...
            u0[i][j].mu  = 1.0 * u0[i][j].rho;
            u0[i][j].mv  = 0.0 * u0[i][j].rho;
            // u0[i][j].mu  = -1e2*y[j]/sqrt(r)*sin(sqrt(r) * M_PI) * u0[i][j].rho;
            // u0[i][j].mv  =  1e2*x[i]/sqrt(r)*sin(sqrt(r) * M_PI) * u0[i][j].rho;
...
vector<vector<ConsState>> NSE_Operator(const vector<vector<ConsState>>& U) {
    ...
    for (int i = 1; i < Nx-1 ; i++) {
        for (int j = 1; j < Ny-1 ; j++) {
            ...
            double dP_dy  = cs * cs * ( U[i][j+1].rho - U[i][j-1].rho) / (2.0 * dy);
            
            // Time differential
            dUdt[i][j].rho  = - dmu_dx - dmv_dy;
